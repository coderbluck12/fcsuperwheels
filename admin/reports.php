...
$stmt = $pdo->prepare("SELECT SUM(sale_price) as total_sales FROM vehicles WHERE status = 'Sold' AND sale_date BETWEEN ? AND ?");
...
$stmt = $pdo->prepare("SELECT SUM(sale_price - purchase_price) as gross_profit FROM vehicles WHERE status = 'Sold' AND sale_date BETWEEN ? AND ?");
$stmt->execute([$start_date, $end_date]);
$gross_profit = $stmt->fetch()['gross_profit'] ?? 0;
$net_profit = $gross_profit - $total_expenses;

// Get recent sales
$stmt = $pdo->prepare("SELECT * FROM vehicles WHERE status = 'Sold' AND sale_date BETWEEN ? AND ? ORDER BY sale_date DESC LIMIT 5");
$stmt->execute([$start_date, $end_date]);

<?php
// Enable Error Reporting so we can see issues
...
// --- REPLACE YOUR EXISTING POST/DELETE LOGIC WITH THIS ---
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_logs'])) {
    // Ensure we have a connection
    if (!isset($pdo) || !$pdo) { $pdo = get_db_connection(); }

    if ($pdo) {
        $days_to_keep = (int)$_POST['days_to_keep'];
        
        try {
            // FIX: Handle "0" to delete everything for testing
            if ($days_to_keep === 0) {
                $stmt = $pdo->prepare("DELETE FROM access_log");
                $stmt->execute();
            } else {
            ...
            // Log the action
            log_access('CLEAN_ACCESS_LOG', 'access_log.php', null, null, "Deleted $deleted_count log entries");
            
            $message = '<div class="alert alert-success alert-dismissible fade show" role="alert">
                Success! Deleted ' . $deleted_count . ' log entries.
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <select name="days_to_keep" class="form-select">
                            <option value="0" style="color: red; font-weight: bold;">DELETE EVERYTHING (0 days)</option>

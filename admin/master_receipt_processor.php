...
$visibility        = validate_input($_POST['visibility'] ?? 'yes');

# optional override of creation time
if (!empty($_POST['time_created'])) {
    $time_created = validate_input($_POST['time_created']);
} else {
...
if ($stmt->rowCount() > 0) {
    # redirect using only encrypted id
    $encrypted = encryptData($lastId, '31081990');
    redirect_to("master_view_receipt.php?prefix_receipt_number=" . urlencode($encrypted));
} else {

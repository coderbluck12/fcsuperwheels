...
if(isset($_GET['quick_button']))
{
	if(isset($_GET['receipt_number'])&&!empty($_GET['receipt_number']))
	{
	$search_input = validate_input($_GET['receipt_number']);
	}
	else
	{
...
<?php  include_once('inc/menu.php'); ?>
 ...
 <section class="au-breadcrumb2">
                          ...
                          <div class="au-breadcrumb-content">
                              <div class="au-breadcrumb-left">
                                  ...
                                  <ul class="list-unstyled list-inline au-breadcrumb__list">
                                      ...
                                      <li class="list-inline-item">Search result</li>
                                  </ul>
                              </div>
                             <a href="dashboard.php"><button onclick="report_all.php" class="au-btn au-btn--small au-btn--blue m-b-20">BACK TO DASHBOARD</button></a>
        ...
        // Prepare the SQL query with the concatenation and WHERE clause
		$stmt = $pdo->prepare("SELECT * FROM main_receipt WHERE CONCAT(id, prefix_receipt_number) = :searchinput");
		$stmt->bindParam(':searchinput', $search_input, PDO::PARAM_STR);
		$stmt->execute();
		...
		if ($rowCount > 0)
		{
			$comment = '<a style="color:reen;text-align:center;"><b>Invoice with number '.$search_input.' was found in record</b></a>';
		}
		else
		{
			$comment = '<a style="color:red;text-align:center;"><b>Invoice with number '.$search_input.' not in record</b></a>';
		}
		?>
		...
		{
		?>
        <h2 class="mt-5 mb-4">Search result - <?php  echo $rowCount; ?> receipt found</h2>
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            <tr>
                ...
                <td>
                    <a href="view_receipt.php?prefix_receipt_number=<?php echo urlencode($encrypted_id); ?>" class="au-btn au-btn--small au-btn--green m-b-20" style="" target="_blank">Open</a>
                </td>
            </tr>
		...
		else
		{
			?>
			<h2 class="mt-5 mb-4">Search result - <?php  echo $rowCount; ?> receipt found</h2>
			<h3> Receipt #<?php echo $search_input; ?> is not in our record</h3>
